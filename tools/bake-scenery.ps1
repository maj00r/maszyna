...
param(
    ...
    # bake only these, instead of everything that lacks a .sbt
    [string[]] $Scenery,

    # stop starting new bakes once the disk has less than this much room left. a single
    # .sbt has been seen to pass a gigabyte, so leave headroom
    [int] $MinFreeGB = 10,
...
# a .sbt runs to several hundred MB and occasionally past a gigabyte, so say plainly
...
while ($queue.Count -gt 0 -or $running.Count -gt 0) {
    ...
    foreach ($id in @($running.Keys)) {
        ...
        $sbt = Join-Path $sceneryDir ([System.IO.Path]::GetFileNameWithoutExtension($entry.Name) + ".sbt")

        if ($entry.Proc.ExitCode -eq 0 -and (Test-Path $sbt)) {
            $size = (Get-Item $sbt).Length / 1MB
