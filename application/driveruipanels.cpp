...
debug_panel::update_section_vehicle( std::vector<text_line> &Output ) {
    ...
    std::snprintf(
        ...
        // brakes; valve: odcięty u biernej obsady / po Activation
        mover.fBrakeCtrlPos,
...
debug_panel::update_section_scantable( std::vector<text_line> &Output ) {
	...
	// najbliższy pojazd na trasie wstawiamy wg dystansu między wpisy tabelki
	...
	for( std::size_t i = 0; i < speedtablesize; ++i ) {
		...
		// wygaszone / poza zakresem dają pusty string; nie przerywamy - luki bywają w środku tabelki
		if( scanline.empty() ) { continue; }
		if( obstaclepending
		 && mechanik.Obstacle.distance < mechanik.TableDistance( i ) ) {
	...
	if( obstaclepending ) {
		Output.emplace_back( Bezogonkow( obstacleline ), Global.UITextColor ); // przeszkoda za ostatnim wpisem
	}
	if( Output.size() == 1 ) {
