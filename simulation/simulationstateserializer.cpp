...
state_serializer::deserialize_terrain(cParser &Input, scene::scratch_data &Scratchpad)
{
	...
	{
		// a scenery may name someone else's .sbt instead of relying on its own, so that
		...
		{
			// this scenery does not own the named .sbt and so cannot rebuild it. the terrain
