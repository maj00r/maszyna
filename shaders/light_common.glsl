...
float calc_shadow()
{
	...
	for (cascade = 0U; cascade < MAX_CASCADES; cascade++)
		if (distance <= cascade_end[cascade])
			break;
	// Past the last cascade there is no map to sample, and the loop leaves the
	// index one beyond the array - reading it here is out of bounds.
	...
	// Only enough to absorb depth quantisation. The depth pass culls front
	// faces and applies glPolygonOffset, whose first argument already scales
	// with slope, so biasing the receiver again - especially by slope - just
	// lifts shadows off their contact points and leaks light through them.
	float bias = 0.00002f * float(cascade + 1U);
